...
@Service
@AllArgsConstructor
public class AppointmentCreateResponseStep implements StepRunnerModel<AppointmentData, AppointmentMetadata> {
    @Override
    public void runStep(StepStore<AppointmentData, AppointmentMetadata> stepStore) {
        if (stepStore.getSuccess()) {
            stepStore.addMessage("Appointments were created correctly!");
        }
    }
}
