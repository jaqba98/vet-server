...
@Service
@AllArgsConstructor
public class MedicationCreateResponseStep implements StepRunnerModel<MedicationData, MedicationMetadata> {
    @Override
    public void runStep(StepStore<MedicationData, MedicationMetadata> stepStore) {
        if (stepStore.getSuccess()) {
            stepStore.addMessage("A new medication was created correctly!");
        }
    }
}
